...
async function loadSuggestions() {
    ...
    try {
        ...
        suggestionCards.innerHTML = result.suggestions.map(item => {
            ...
            const summaryHTML = window.canAdmin && item.admin_summary ? `
                <div class="suggestion-response admin-summary">
                    <h3>Resumo melhorado</h3>
                    <p>${item.admin_summary}</p>
                    <small>Custo estimado: R$ ${typeof item.estimated_cost === 'number' ? item.estimated_cost.toFixed(2) : 'N/A'} • Duração: ${item.estimated_duration || 'N/A'}</small>
                </div>
            ` : '';
            ...
            const adminResponseForm = window.canAdmin ? `
                <form class="response-form" data-id="${item.id}">
                    ...
                    <button type="submit">${item.response ? 'Editar resposta' : 'Enviar resposta'}</button>
                </form>
            ` : '';
